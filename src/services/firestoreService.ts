        ...
        if (!snapshot.empty) {
          ...
          snapshot.forEach((d) => {
            if (!isDocDeleted(d.id)) {
              list.push({
                id: d.id,
                ...(d.data() as Omit<UmrahPackage, "id">),
              });
            }
          });
          callback(list);
...
export const subscribeGalleryItems = (
  ...
  try {
    ...
    return onSnapshot(
      ...
      (error) => {
        console.warn(
          "Firestore Gallery snapshot error, using fallback:",
          error,
        );
        const filtered = fallbackData.filter((item) => !isDocDeleted(item.id));
        callback(filtered);
...
export const subscribeReviews = (
  ...
  try {
    ...
    return onSnapshot(
      ...
      (error) => {
        console.warn(
          "Firestore Reviews snapshot error, using fallback:",
          error,
        );
        const filtered = fallbackData.filter((item) => !isDocDeleted(item.id));
        callback(filtered);
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => {
          if (!isDocDeleted(d.id)) {
            list.push({
              id: d.id,
              ...(d.data() as Omit<BookingInquiry, "id">),
            });
          }
        });
        // Sort latest first
...
export const subscribeHomePageConfig = (
  ...
  try {
    ...
    return onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          const merged: HomePageConfig = {
            ...
            trustBadges:
              Array.isArray(data.trustBadges) && data.trustBadges.length > 0
                ? data.trustBadges
                : fallbackConfig.trustBadges,
            servicesList:
              Array.isArray(data.servicesList) && data.servicesList.length > 0
                ? data.servicesList
                : fallbackConfig.servicesList,
            accordionItems:
              Array.isArray(data.accordionItems) &&
              data.accordionItems.length > 0
                ? data.accordionItems
                : fallbackConfig.accordionItems,
            accreditationsList:
              Array.isArray(data.accreditationsList) &&
              data.accreditationsList.length > 0
                ? data.accreditationsList
                : fallbackConfig.accreditationsList,
            featuresListBn:
              Array.isArray(data.featuresListBn) &&
              data.featuresListBn.length > 0
                ? data.featuresListBn
                : fallbackConfig.featuresListBn,
            featuresListEn:
              Array.isArray(data.featuresListEn) &&
              data.featuresListEn.length > 0
                ? data.featuresListEn
                : fallbackConfig.featuresListEn,
            typewriterMessagesBn:
              Array.isArray(data.typewriterMessagesBn) &&
              data.typewriterMessagesBn.length > 0
                ? data.typewriterMessagesBn
                : fallbackConfig.typewriterMessagesBn,
            typewriterMessagesEn:
              Array.isArray(data.typewriterMessagesEn) &&
              data.typewriterMessagesEn.length > 0
                ? data.typewriterMessagesEn
                : fallbackConfig.typewriterMessagesEn,
          };
          callback(merged);
        } else {
      ...
      (error) => {
        console.warn(
          "Firestore HomePageConfig snapshot error, using fallback:",
          error,
        );
        callback(fallbackConfig);
      },
...
export const seedAllDefaultDataToFirestore = async (): Promise<{
  success: boolean;
  message: string;
}> => {
  try {
    ...
    await saveSiteSettings({
      ...
      hadithEn:
        'The Prophet (pbuh) said: "One Umrah to the next is an expiation for the sins committed between them." (Bukhari & Muslim)',
      noticeBn:
    ...
    return {
      success: true,
      message: "সকল ডাটা সফলভাবে ফায়ারবেস ক্লাউড ডাটাবেজে সংরক্ষিত হয়েছে!",
    };
  } catch (err: any) {
    console.error("Error seeding Firestore data:", err);
    return {
      success: false,
      message: "ডাটা সংরক্ষণ করতে সমস্যা হয়েছে: " + err.message,
    };
  }
};
