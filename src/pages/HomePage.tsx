...
import { Language, PageId, UmrahPackage, HajjPackage, BlogPost, AgencyInfo, HomePageConfig } from '../types';
...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  const messages = isBn 
    ? (config.typewriterMessagesBn && config.typewriterMessagesBn.length > 0 ? config.typewriterMessagesBn : initialHomePageConfig.typewriterMessagesBn)
    : (config.typewriterMessagesEn && config.typewriterMessagesEn.length > 0 ? config.typewriterMessagesEn : initialHomePageConfig.typewriterMessagesEn);
      ...
      {config.showHeroSection !== false && (
        <section className="relative bg-[#03180D] text-white pt-3 sm:pt-4 lg:pt-5 pb-28 sm:pb-36 lg:pb-44 overflow-hidden border-b-4 border-[#D4AF37]">
          ...
          <div 
            ...
            style={{ backgroundImage: `url('${config.heroBgImage || initialHomePageConfig.heroBgImage}')` }}
          />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-8 items-start">
              ...
              <div className="lg:col-span-7 space-y-4 sm:space-y-5 lg:space-y-5.5 text-center lg:text-left flex flex-col justify-between">
                ...
                <p className="text-xs sm:text-sm md:text-base text-emerald-100/90 leading-relaxed max-w-2xl">
                  {isBn 
                    ? (config.heroSubtitleBn || 'ঢাকা পান্থপথ প্রধান কার্যালয় থেকে সরাসরি বাংলাদেশ সরকার ও সৌদি ধর্ম মন্ত্রণালয়ের নিয়ম মেনে স্বচ্ছতার সাথে পরিচালিত।')
                    : (config.heroSubtitleEn || 'Operating directly from Panthapath main office with 100% Shariah compliance and government licensing.')
                  }
                </p>

                {/* Quick Stat Badges Row */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-left">
                  {(config.stats && config.stats.length > 0 ? config.stats : initialHomePageConfig.stats).map((stat) => (
                    <div key={stat.id} className="bg-white/10 backdrop-blur-md p-3 sm:p-3.5 rounded-2xl border border-white/15 shadow-sm hover:border-[#D4AF37]/50 transition-colors">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3.5 pt-0.5 text-xs text-emerald-100 font-medium">
                  {(config.trustBadges && config.trustBadges.length > 0 ? config.trustBadges : initialHomePageConfig.trustBadges).map((badge) => (
                    <span key={badge.id} className="flex items-center justify-center gap-2 bg-emerald-950/80 border border-emerald-700/60 px-3.5 py-2 rounded-full shadow-xs text-center">
                ...
                <div className="flex flex-wrap sm:flex-nowrap items-center justify-between sm:justify-start gap-4 sm:gap-6 lg:gap-8 pt-1">
                  ...
                  >
                    ...
                    <span>{isBn ? (config.heroBtnUmrahBn || 'উমরাহ্ প্যাকেজসমূহ') : (config.heroBtnUmrahEn || 'Umrah Packages')}</span>
                  ...
                  >
                    <span className="text-base">🕌</span>
                    <span>{isBn ? (config.heroBtnHajjBn || 'পবিত্র হজ্ব ২০২৭') : (config.heroBtnHajjEn || 'Holy Hajj 2027')}</span>
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 pt-2">
          <div className="text-center mb-5">
            <span className="text-xs font-bold text-[#B38712] uppercase tracking-wider block font-sans">
              {isBn ? (config.importantTaglineBn || 'Important Guidance') : (config.importantTaglineEn || 'Important Guidance')}
            </span>
            <h2 className="text-2xl sm:text-3xl font-black text-[#0D472B]">
              {isBn ? (config.importantTitleBn || 'একটু জেনে নিন...') : (config.importantTitleEn || 'Key Information for Pilgrims')}
            </h2>
          </div>
          ...
          <div className="relative bg-gradient-to-br from-rose-50/80 via-[#FAF4EC] to-emerald-50/80 p-6 sm:p-10 rounded-3xl border border-rose-200/80 shadow-md overflow-hidden text-slate-800 space-y-4">
            ...
            <p className="text-xs sm:text-sm text-slate-700 leading-relaxed text-justify font-medium">
              {isBn ? (
                config.importantDescBn || 'যাঁরা পবিত্র উমরাহ্ ও হজ্ব পালন করতে চান, আমরা সরাসরি বাংলাদেশ সরকার ও সৌদি আরব ধর্ম মন্ত্রণালয়ের নিয়ম মেনে স্বচ্ছতার সাথে সেবা প্রদান করি। বিশেষত ঢাকা, কুমিল্লা, সিলেট ও চট্টগ্রামসহ সারা দেশের সম্মানিত হাজীদের এয়ার টিকিট, ভিসা প্রসেসিং এবং হারামের নিকটবর্তী থ্রি-স্টার / ফাইভ-স্টার হোটেল সুবিধা নিশ্চিত করা হয়।'
              ) : (
                config.importantDescEn || 'For pilgrims intending to perform Holy Umrah or Hajj, we strictly adhere to the guidelines of the Ministry of Religious Affairs Bangladesh and Saudi Arabia. We ensure seamless air tickets, visa processing, and guaranteed 3-star/5-star hotels near Haram.'
              )}
            </p>
            ...
            <div className="bg-white/90 p-4.5 rounded-2xl border border-rose-200/60 space-y-2 text-xs font-semibold text-slate-800 shadow-2xs">
              <p className="font-bold text-[#0D472B] text-sm flex items-center gap-2">
                ...
                <span>{isBn ? (config.importantBoxTitleBn || 'গুরত্বপূর্ণ যে বিষয়গুলো খেয়াল রাখবেন:') : (config.importantBoxTitleEn || 'Key Prerequisites:')}</span>
              </p>
              <ul className="list-disc list-inside space-y-1.5 text-slate-700 pl-1">
                {(isBn 
                  ? (config.importantBoxPointsBn && config.importantBoxPointsBn.length > 0 ? config.importantBoxPointsBn : initialHomePageConfig.importantBoxPointsBn)
                  : (config.importantBoxPointsEn && config.importantBoxPointsEn.length > 0 ? config.importantBoxPointsEn : initialHomePageConfig.importantBoxPointsEn)
                ).map((pt, idx) => (
                  <li key={idx}>{pt}</li>
                ))}
              </ul>
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <button
                onClick={() => onOpenBookingModal(
                  ...
                  isBn ? 'পবিত্র উমরাহ্ ও হজ্ব সরাসরি পরামর্শ ও বুকিং তথ্য' : 'Holy Umrah & Hajj Direct Booking & Guidance'
                )}
              ...
              >
                <span>{isBn ? (config.importantBtnTextBn || 'বিস্তারিত জানতে যোগাযোগ করুন') : (config.importantBtnTextEn || 'Contact for More Details')}</span>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-4">
          <div className="text-center max-w-2xl mx-auto space-y-2 mb-8">
            <span className="text-xs font-extrabold text-[#B38712] uppercase tracking-wider block font-sans">
              {isBn ? (config.servicesTaglineBn || 'Our Core Services') : (config.servicesTaglineEn || 'Our Core Services')}
            </span>
            <h2 className="text-2xl sm:text-3xl font-black text-[#0D472B]">
              {isBn ? (config.servicesTitleBn || 'আমাদের প্রধান সেবাসমূহ') : (config.servicesTitleEn || 'Our Major Services')}
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {(config.servicesList && config.servicesList.length > 0 ? config.servicesList : initialHomePageConfig.servicesList).map((srv) => (
              <div key={srv.id} className="bg-white rounded-3xl p-6 border border-slate-200 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between group hover:-translate-y-1">
                ...
                >
                  <span>{isBn ? (srv.btnTextBn || 'বিস্তারিত দেখুন') : (srv.btnTextEn || 'View Details')}</span>
        ...
        <section className="bg-emerald-950/5 py-12 border-y border-emerald-900/10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
              <div>
                <span className="text-xs font-bold text-[#B38712] uppercase tracking-wider block font-sans">
                  {isBn ? (config.featuredTaglineBn || 'Featured Packages') : (config.featuredTaglineEn || 'Featured Packages')}
                </span>
                <h2 className="text-2xl sm:text-3xl font-black text-[#0D472B]">
                  {isBn ? (config.featuredTitleBn || 'আমাদের বিশেষ আকর্ষণীয় প্যাকেজসমূহ') : (config.featuredTitleEn || 'Featured Umrah & Hajj Packages')}
                </h2>
              </div>
              ...
              <div className="flex items-center gap-2 bg-white p-1 rounded-full border border-slate-200 shadow-2xs text-xs font-bold">
                ...
                >
                  {isBn ? (config.featuredFilterAllBn || 'সব') : (config.featuredFilterAllEn || 'All')}
                </button>
                <button
                ...
                >
                  {isBn ? (config.featuredFilterUmrahBn || 'উমরাহ্') : (config.featuredFilterUmrahEn || 'Umrah')}
                </button>
                <button
                ...
                >
                  {isBn ? (config.featuredFilterHajjBn || 'হজ্ব') : (config.featuredFilterHajjEn || 'Hajj')}
                </button>
              </div>
            </div>
            ...
            <div className="text-center pt-8">
              ...
              >
                <span>{isBn ? (config.featuredExploreBtnBn || 'সকল প্যাকেজ একসাথে দেখুন') : (config.featuredExploreBtnEn || 'Explore All Travel Packages')}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-md overflow-hidden">
            <div className="bg-gradient-to-r from-slate-100 via-emerald-50/80 to-slate-100 px-6 py-4 border-b border-slate-200">
              <h3 className="text-base font-black text-[#0D472B]">
                {isBn ? (config.accordionTitleBn || 'আরও দেখুন & প্রয়োজনীয় নির্দেশিকা') : (config.accordionTitleEn || 'Explore More Guides & Instructions')}
              </h3>
            </div>

            <div className="divide-y divide-slate-100">
              {(config.accordionItems && config.accordionItems.length > 0 ? config.accordionItems : initialHomePageConfig.accordionItems).map((item, idx) => {
          ...
          <div className="bg-gradient-to-br from-[#0D472B] via-[#083620] to-[#03180D] text-white rounded-3xl p-6 sm:p-10 shadow-2xl border-2 border-[#D4AF37] relative overflow-hidden">
            <div className="text-center max-w-xl mx-auto mb-8 space-y-2">
              <h2 className="text-2xl sm:text-3xl font-black font-sans">
                {isBn ? (config.featuresTitleBn || 'আমাদের বিশেষ বৈশিষ্ট্যসমূহ') : (config.featuresTitleEn || 'Our Distinctive Features')}
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {(isBn 
                ? (config.featuresListBn && config.featuresListBn.length > 0 ? config.featuresListBn : initialHomePageConfig.featuresListBn)
                : (config.featuresListEn && config.featuresListEn.length > 0 ? config.featuresListEn : initialHomePageConfig.featuresListEn)
              ).map((feature, idx) => (
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-md">
            <div className="text-center mb-6">
              <span className="text-xs font-bold text-[#B38712] uppercase tracking-wider block font-sans">
                {isBn ? (config.accreditationsTaglineBn || 'Govt. Approved & Licensed Agency') : (config.accreditationsTaglineEn || 'Govt. Approved & Licensed Agency')}
              </span>
              <h3 className="text-xl font-black text-[#0D472B]">
                {isBn ? (config.accreditationsTitleBn || 'আমাদের সরকারি স্বীকৃতি ও আইএটিএ সদস্যপদ') : (config.accreditationsTitleEn || 'Our Government Approvals & Licenses')}
              </h3>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              {(config.accreditationsList && config.accreditationsList.length > 0 ? config.accreditationsList : initialHomePageConfig.accreditationsList).map((acc) => (
                <div key={acc.id} className="p-4.5 rounded-2xl bg-emerald-50/80 border border-emerald-200/80 space-y-1">
                  {renderAccreditationIcon(acc.iconName)}
                  <p className="text-xs font-bold text-[#0D472B]">{isBn ? acc.titleBn : acc.titleEn}</p>
                  <p className="text-[11px] text-slate-600 font-sans font-semibold">{isBn ? acc.subBn : acc.subEn}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#052917] text-white rounded-3xl p-6 sm:p-8 border-2 border-[#D4AF37] shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center md:text-left">
              <h3 className="text-xl sm:text-2xl font-black font-sans">
                {isBn ? (config.contactBannerTitleBn || 'যেকোনো জিজ্ঞাসায় সরাসরি আমাদের হটলাইনে যোগাযোগ করুন') : (config.contactBannerTitleEn || 'Call Us Directly for Any Flight or Package Query')}
              </h3>
              <p className="text-xs sm:text-sm text-emerald-200">
                {isBn ? (config.contactBannerSubtitleBn || 'প্রধান অফিস: পশ্চিম পান্থপথ জামে-মসজিদের বিপরীত পাশে, ঢাকা-১২১৫') : (config.contactBannerSubtitleEn || 'Panthapath Main Office, Dhaka-1215')}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                ...
                <span>{isBn ? toBengaliDigits(config.contactBannerHotline || agencyInfo.hotline) : (config.contactBannerHotline || agencyInfo.hotline)}</span>
                <span className="text-sm sm:text-base font-bold text-emerald-900 font-bengali">
                  ({isBn ? (config.contactBannerHotlineLabelBn || 'হোয়াটসঅ্যাপ & ইমো') : (config.contactBannerHotlineLabelEn || 'WhatsApp & Imo')})
                </span>
              </a>
              ...
              >
                {isBn ? (config.contactBannerCtaBtnBn || getTranslation(lang, 'contactCTA')) : (config.contactBannerCtaBtnEn || 'Contact Us')}
              </button>
            </div>
          </div>
        </section>
