...
import { Star, MessageSquare, Plus, CheckCircle2, ShieldCheck, User } from 'lucide-react';
              ...
              <div className="flex items-center justify-between">
                ...
                {rev.verified && (
                  <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
                    ...
                    <span>{lang === 'bn' ? 'যাচাইকৃত ক্লায়েন্ট' : 'Verified Client'}</span>
                  </span>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl border border-[#D4AF37] space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">{getTranslation(lang, 'formName')}</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">{lang === 'bn' ? 'স্থান / এলাকা' : 'Location'}</label>
                <input
                  type="text"
                  value={location}
                  onChange={(e) => setLocation(e.target.value)}
                  placeholder={lang === 'bn' ? 'যেমন: ধানমন্ডি, ঢাকা' : 'e.g. Dhaka'}
                  className="w-full border border-slate-300 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#0D472B]"
                />
              </div>

              <div>
                <label className="block font-bold text-slate-700 mb-1">{getTranslation(lang, 'formService')}</label>
                <input
                  type="text"
                  value={service}
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">{lang === 'bn' ? 'রেটিং' : 'Rating'}</label>
                <select
                  value={rating}
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">{lang === 'bn' ? 'আপনার অভিজ্ঞতা লিখুন' : 'Your Review'}</label>
                <textarea
                  rows={3}
                  required
                  value={comment}
