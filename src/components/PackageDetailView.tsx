...
function getHolySiteCaption(
  ...
  const matchedDriveItem = ALL_HAJJ_DRIVE_PHOTOS.find((item) =>
    imgUrl.includes(item.driveId),
  );
  if (matchedDriveItem) {
  ...
  if (
    ...
    return isBn
      ...
      : "Holy Madinah (Al-Masjid an-Nabawi)";
  }
  ...
  if (
    ...
    lower.includes("rahmah")
  ) {
    return isBn
      ? "জেদ্দার বিখ্যাত আল-রহমাহ ফ্লোটিং মসজিদ"
      : "Jeddah Al-Rahmah Floating Mosque";
  }
  ...
  if (serviceType === "Hajj") {
    ...
    return isBn
      ? hajjSitesBn[index % hajjSitesBn.length]
      : hajjSitesEn[index % hajjSitesEn.length];
  }
  return isBn
    ? `গ্যালারি ছবি ${toBengaliDigits(index + 1)}`
...
export const PackageDetailView: React.FC<PackageDetailViewProps> = ({
  ...
  const currentImage =
    currentGalleryImages[activeImageIndex] ||
    currentGalleryImages[0] ||
    pkg.image;
  const currentCaption = getHolySiteCaption(
    currentImage,
    activeImageIndex,
    isBn,
    pkg.serviceType,
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6 font-bengali animate-fadeIn">
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        >
          ...
          <span>
            {isBn ? "← পূর্ববর্তী তালিকায় ফিরে যান" : "← Back to Package List"}
          </span>
        </button>
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
          ...
          <span>/</span>
          <span className="text-[#0D472B] truncate max-w-[180px] sm:max-w-xs">
            {title}
          </span>
        </div>
      </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/80 shadow-sm space-y-4">
            ...
            >
              ...
              <div className="absolute top-3 left-3 bg-[#052917]/90 backdrop-blur-md text-white text-xs sm:text-sm px-3.5 py-1.5 rounded-xl font-bold border border-[#D4AF37]/50 shadow-lg flex items-center gap-2 z-10">
                ...
                <span className="font-extrabold tracking-wide text-[#FAF8F5]">
                  {currentCaption}
                </span>
              </div>
                ...
                <div className="absolute top-3 right-3 flex items-center gap-2 z-10">
                  <button
                    ...
                    title={
                      isAutoPlaying
                        ? isBn
                          ? "অটো-স্লাইড বন্ধ করুন"
                          : "Pause Auto-slide"
                        : isBn
                          ? "অটো-স্লাইড চালু করুন"
                          : "Start Auto-slide"
                    }
                  >
                    {isAutoPlaying ? (
                      <>
                        ...
                        <span className="text-[11px] font-semibold hidden sm:inline">
                          {isBn ? "অটো পরিবর্তন চালু" : "Auto Sliding"}
                        </span>
                      </>
                    ) : (
                      <>
              ...
              <span className="absolute bottom-3 right-3 bg-black/80 backdrop-blur-md text-white text-xs px-3 py-1.5 rounded-full font-bold border border-white/20 flex items-center gap-1.5 z-10">
                ...
                <span>
                  {isBn
                    ? `${toBengaliDigits(activeImageIndex + 1)} / ${toBengaliDigits(currentGalleryImages.length)}`
              ...
              <div className="space-y-1.5">
                ...
                >
                  {currentGalleryImages.map((imgUrl, idx) => {
                    const caption = getHolySiteCaption(
                      imgUrl,
                      idx,
                      isBn,
                      pkg.serviceType,
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg sm:text-xl font-black font-sans">
                  {isBn
                    ? "দিনের বিস্তারিত কর্মপরিকল্পনা (Day wise itinerary)"
                    : "Day wise itinerary"}
                </span>
              </div>
            ...
            {openAccordions.itinerary && (
              <div className="p-5 sm:p-6 space-y-4 bg-white">
                {pkg.itinerary.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-4 sm:p-5 rounded-2xl border border-slate-200/80 bg-[#FAF8F5] hover:border-[#0D472B]/30 hover:bg-[#FAF6EF] space-y-2.5 transition-all"
                  >
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg sm:text-xl font-black font-sans">
                  {isBn
                    ? "প্যাকেজে অন্তর্ভুক্ত সুবিধাসমূহ (Inclusions)"
                    : "Package Inclusions"}
                </span>
              </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg sm:text-xl font-black font-sans">
                  {isBn
                    ? "বাতিল ও রিফান্ড নীতি (Cancellation Policy)"
                    : "Cancellation Policy"}
                </span>
              </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-lg sm:text-xl font-black font-sans">
                  {isBn
                    ? "সাধারণ নিয়মাবলী ও শর্তসমূহ (General Terms)"
          ...
          <div className="pt-4">
            ...
            >
              ...
              <span>
                {isBn ? "← প্যাকেজ তালিকায় ফিরে যান" : "← Back to Package List"}
              </span>
            </button>
          </div>
        </div>
          ...
          <div className="lg:sticky lg:top-24 bg-white rounded-3xl p-6 border-2 border-red-200 shadow-xl space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-1.5 text-xs font-black text-[#8B0000] uppercase tracking-wider">
                ...
                <span>
                  {isBn ? "বিশেষ অফার বুকিং" : "Special Offer Booking"}
                </span>
              </div>
            ...
            {pkg.priceBDT > 0 && (
              <div className="space-y-1 bg-red-50/60 p-5 rounded-2xl border border-red-100 text-center">
                <span className="text-xs text-slate-500 font-bold block">
                  {isBn
                    ? "সর্বমোট প্যাকেজ মূল্য (প্রতি জন)"
                    : "Price Per Person"}
                </span>
                ...
                <span className="text-[11px] text-slate-500 block">
                  {isBn
                    ? "ট্যাক্স ও সকল সেবা ফি অন্তর্ভুক্ত"
              ...
              <div className="space-y-2.5 text-xs text-slate-700">
                ...
                {pkg.hotelsBn.map((hotel, idx) => (
                  ...
                  >
                    ...
                    <span className="font-semibold text-slate-800">
                      {isBn ? hotel : pkg.hotelsEn ? pkg.hotelsEn[idx] : hotel}
                    </span>
                  </div>
                ))}
                ...
                <div className="flex items-center gap-2.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <span className="font-medium text-slate-800">
                    {isBn
                      ? "সুস্বাদু ৩ বেলা দেশীয় খাবার"
                      : "Full Catering Meal Support"}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <span className="font-medium text-slate-800">
                    {isBn
                      ? "এসি লাক্সারি বাস ও জিয়ারাহ্"
                      : "AC Bus Transport & Ziyarah"}
                  </span>
                </div>
              </div>
            )}
            ...
            {pkg.priceBDT > 0 && (
              ...
              >
                <span>
                  {isBn
                    ? "এখনই বুক করতে যোগাযোগ করুন (Select Offer)"
                    : "Contact to Book Now (Select Offer)"}
                </span>
              </button>
            )}
            ...
            <div className="border-t border-slate-100 pt-4 space-y-2.5">
              ...
              >
                ...
                <span>
                  {isBn ? "হোয়াটসঅ্যাপে মেসেজ পাঠান" : "WhatsApp Inquiry"}
                </span>
              </a>
